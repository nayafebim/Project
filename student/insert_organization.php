...
<body class="bg-gradient-to-br from-green-700 to-teal-500 text-white min-h-screen">
    ...
    <script>
        ...
        document.getElementById('modal-submit-btn').addEventListener('click', function(event) {
            ...
            // ปิดใช้งานปุ่มเพื่อป้องกันการคลิกซ้ำ
            this.disabled = true;

            // เรียกฟังก์ชันเพิ่มข้อมูลประเภทใหม่
            addNewType();
        });
        ...
        function addNewType() {
            ...
            $.ajax({
                type: 'POST',
                url: 'ajax/insert_type.php', // ไฟล์ PHP สำหรับการบันทึกประเภทใหม่
                data: {
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = 'insert_organization.php';
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
                            title: 'เกิดข้อผิดพลาด!',
                            text: response.message, // แสดงข้อความที่ได้จาก PHP เช่น "ชื่อประเภทนี้มีอยู่แล้วในระบบ"
                            confirmButtonText: 'ตกลง'
                        });
                    }
                },
