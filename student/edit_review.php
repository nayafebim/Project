            ...
            ?>
                ...
                <form method="POST">
                    <div class="mb-4">
                        ...
                        <textarea id="comment" name="review_text" class="w-full p-3 rounded-lg border border-gray-300 focus:outline-none" rows="4"><?= $row_org["review_text"]; ?></textarea>
                        <div class="flex justify-center text-center mt-5">
                            <a href="indexreview.php" class="bg-red-600 text-white px-6 py-2 rounded-lg mr-5"><i class="fa-solid fa-arrow-left-long mr-1"></i> ย้อนกลับ</a>
                            <button type="submit" name="update_review" class="bg-green-600 text-white px-6 py-2 rounded-lg"><i class="fas fa-save"></i> บันทึกข้อมูล</button>
                        </div>
                    </div>
                </form>
            <?php } ?>
...
$sql = "SELECT * FROM intern WHERE student_id = '$User_ID'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($review_text)) {
        ...
        $sql = "UPDATE `review` SET `review_text`='$review_text',`review_date`='$date' WHERE student_id = $student_id";
