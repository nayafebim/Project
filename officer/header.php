...
if ($stmt = $conn->prepare($sql)) {
    ...
    $officerData = $result->fetch_assoc();
    $stmt->close();
} else {
...
<header class="bg-green-800 p-4 flex justify-between items-center">
    <div class="flex items-center space-x-4">
        ...
        <a href="docorganize.php" class="text-yellow-300 hover:text-gray-300">หนังสือขอความอนุเคราะห์</a>
        <!-- <a href="formdoc.php" class="text-yellow-300 hover:text-gray-300">แบบฟอร์มเอกสาร</a> -->
        ...
        <a href="insertcsv.php" class="text-yellow-300 hover:text-gray-300">นำเข้าข้อมูล</a>
    </div>
    <div class="flex items-center space-x-2 relative">
        <span class="text-white"><?= htmlspecialchars($officerData['officer_prefix'] . "" . $officerData['officer_surname'] . " " . $officerData['officer_lastname']) ?></span>
        <img id="avatarButton" type="button" class="w-10 h-10 rounded-full object-cover cursor-pointer" src="../assets/img/officer/<?= $officerData['officer_surname'] ?>.jpg" alt="User dropdown">
        ...
        <div id="userDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 absolute right-0 mt-[150px]">
            <div class="px-4 py-3 text-sm text-gray-900">
                <div><?= htmlspecialchars($officerData['officer_prefix'] . "" . $officerData['officer_surname'] . " " . $officerData['officer_lastname']) ?></div>
                <div><?= htmlspecialchars($officerData['course']) ?></div>
                <div class="font-medium truncate"><?= htmlspecialchars($officerData['officer_email']) ?></div>
            </div>
            <div class="py-1">
