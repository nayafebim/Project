...
$sql = "SELECT COUNT(DISTINCT student.student_id) AS student_count, organization.organization_name, course.course_name , document.Doc_ID
        FROM intern
        ...
        JOIN document ON document.Intern_ID = intern.intern_id
        WHERE organization.organization_id = ? AND student.course = ? AND intern.status_final = ?
        GROUP BY organization.organization_id, organization.organization_name, course.course_name ,  document.Doc_ID
";
if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("iii", $OrganizationID, $Course_ID, $status);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        while ($OrgData = $result->fetch_assoc()) {
            ...
            $document_ID = $OrgData['Doc_ID'];
            $document_thai = convertToThaiNumbers($document_ID);
        }
    } else {
...
$pdf->SetY(40);
// $pdf->Cell(20);
$pdf->Cell(20, 6.5, iconv('UTF-8', 'TIS-620', "ที่ อว.$document_thai/$thaiYearNew"), 0, 1, 'L');
$pdf->SetY(40);
$pdf->Cell(115, 50);
...
$status = 1;
$sql_organization = "SELECT intern.*, organization.*, student.*, course.*, document.* FROM intern 
JOIN organization ON intern.organization_id = organization.organization_id
JOIN student ON intern.student_id = student.student_id
JOIN course ON student.course = course.course_id
JOIN document ON document.Intern_ID = intern.intern_id
